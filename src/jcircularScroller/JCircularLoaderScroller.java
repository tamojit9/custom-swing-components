 ...
 * @author tamojit
 */
public class JCircularLoaderScroller extends JComponent implements Serializable {
    ...
    public JCircularLoaderScroller() {
    ...
    Area getArcShape(int x, int y, int majorAxis, int minorAxis, int startAngle, int angle, int width) {
        ...
        a.subtract(new Area(new Arc2D.Double(x + majorAxis / 2 - w / 2, y + minorAxis / 2 - h / 2, w, h, startAngle, angle+10, Arc2D.PIE)));
        return a;
    }
    ...
    void drawLoadingArc(Graphics2D g2d, int radius, int cx, int cy) {
        int r = radius;
        radius += radius>>1;
        LoadingArc = getArcShape(cx - radius, cy - radius, radius << 1, radius << 1, 0, progress, radius>>1);
        g2d.setColor(getForeground());
        g2d.fill(LoadingArc);
        radius = r;
        LoadingArc.add(getArcShape(cx - radius, cy - radius, radius << 1, radius << 1, progress, 360-progress, 30));
        g2d.fill(LoadingArc);
    }
    ...
    public static void main(String[] argv) throws InterruptedException {
        ...
        final JCircularLoaderScroller jcs = new JCircularLoaderScroller();
        jcs.setSize(300, 300);
        jcs.setLocation(200, 100);
        jcs.setVisible(true);
        jcs.setForeground(Color.LIGHT_GRAY);
